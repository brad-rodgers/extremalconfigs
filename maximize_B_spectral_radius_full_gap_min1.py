...
Unlike the earlier structured program, this implementation imposes NONE of the
following conditions on the gaps:
...
JSON output:
    ...
    python maximize_B_spectral_radius_full_gap_min1.py \
        --csv full_gap_min1_results.csv --json full_gap_min1_results.json

Use more random starts and local validation restarts:

    python maximize_B_spectral_radius_full_gap_min1.py --thorough

Run numerical derivative, invariance, and output-normalization tests only:

    python maximize_B_spectral_radius_full_gap_min1.py --self-test
"""
...
def main() -> None:
    ...
    parser.add_argument("--csv", type=Path, default=Path("full_gap_min1_results_N3_N48.csv"))
