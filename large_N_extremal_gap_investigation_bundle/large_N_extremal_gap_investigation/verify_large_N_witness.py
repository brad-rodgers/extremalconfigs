#!/usr/bin/env python3
"""Independently verify a stored large-N Rayleigh-quotient witness.

This verifier intentionally does *not* import the hierarchical search code.  It
uses only NumPy, the compact plateau profile in ``large_N_gap_search_results``
and the corresponding normalized vector in ``selected_eigenvectors.npz``.
...
a useful cancellation/conditioning diagnostic.  This is O(N^2) time and
O(block_size^2) auxiliary memory; it is intended for occasional validation,
not for optimization.

Examples
--------

    python verify_large_N_witness.py --N 8000 --out witness_N8000.json
    python verify_large_N_witness.py --N 50000 --block-size 256 \
        --out witness_N50000.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--results",
        type=Path,
        default=Path("large_N_gap_search_results.json"),
    )
    parser.add_argument(
        "--vectors",
        type=Path,
        default=Path("selected_eigenvectors.npz"),
    )
    ...
    archive = np.load(args.vectors)
    key = f"v_N{args.N}"
    if key not in archive:
        raise KeyError(f"{args.vectors} does not contain key {key!r}")
    vector = np.asarray(archive[key], dtype=np.complex128)
    ...
    pi_ld = np.longdouble(str(math.pi))
    output = {
        "N": args.N,
        "method": (
