#!/usr/bin/env python3
"""Analyze and plot the output of ``large_N_extremal_gap_search.py``.
...
Usage::
    ...
    python analyze_large_N_gap_results.py \
        --input large_N_gap_search_results.json --out-dir plots
"""
...
def main() -> None:
    ...
    with (args.out_dir / "limit_fit_recomputed.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    ax.plot(fitted_layers_array, 0.5 * fitted_layers_array, ":", label=r"original $\gamma_j=N^{j/2+o(1)}$")
