#!/usr/bin/env python3
"""Large-N numerical search for extremal weighted Hilbert matrices.
...
==============================
The search implemented here is *not* over all N-1 gaps.  It makes the two
assumptions authorized in the accompanying investigation:
...
==============================
``certify`` evaluates a supplied vector with several stricter hierarchical
...
Examples
========
Evaluate and certify a stored profile::

    python large_N_extremal_gap_search.py certify \
        --profile profile_N8000.json --vector eigenvector_N8000.npy \
...
Refine a profile locally::
    ...
    python large_N_extremal_gap_search.py search \
        ...
        --out refined_N8000.json --vector-out eigenvector_N8000.npy

Generate a family of noncommittal geometric *starting profiles*::

    python large_N_extremal_gap_search.py seeds --N 8000 --L 8 \
        --q 2.6,2.9,3.141592653589793,3.5 \
        --central-fraction 0.62,0.68,0.74 --out seeds_N8000.json
"""
...
def certify_vector(
    ...
    """Return cross-rank Rayleigh values and Taylor truncation bounds."""
    ...
    output = {"profile": profile.to_dict(), "hierarchical_certificates": results}
    if do_direct:
        start = time.perf_counter()
...
def load_profile(path: Path) -> PlateauProfile:
    return PlateauProfile.from_dict(json.loads(path.read_text(encoding="utf-8")))
...
def command_certify(args: argparse.Namespace) -> None:
    ...
    certificate = certify_vector(profile, vector, do_direct=args.direct, direct_block_rows=args.direct_block_rows)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    certify = subparsers.add_parser("certify", help="cross-check a profile/vector and optionally do O(N^2) validation")
    certify.add_argument("--profile", type=Path, required=True)
    certify.add_argument("--vector", type=Path)
    certify.add_argument("--out", type=Path, required=True)
    certify.add_argument("--direct", action="store_true")
    certify.add_argument("--direct-block-rows", type=int, default=128)
    add_operator_arguments(certify)
